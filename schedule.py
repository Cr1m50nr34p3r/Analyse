...
def get_events():
    ...
    time_min= datetime.datetime(year=now_dt.year,month=now_dt.month,day=now_dt.day).isoformat() + 'Z'
    time_max= datetime.datetime(year=now_dt.year,month=now_dt.month,day=now_dt.day+1).isoformat() + 'Z'
...
if __name__=="__main__":
    ...
    events_db=[]
    for event in events:
        start = event['start'].get('dateTime',event['start'].get('date')).split("T")[1].split("+")[0]
        end = event['end'].get('dateTime',event['end'].get('date')).split("T")[1].split("+")[0]
        category=event.get('organizer').get('displayName')
        name=event.get('summary')
        events_db.append({ 'start': start,'end': end,'name' : name,'category' : category})
